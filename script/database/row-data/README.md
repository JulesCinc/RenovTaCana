...
- **Emplacement** : les scripts résolvent les chemins depuis leur fichier (`__file__`) et sont exécutables depuis n'importe où ; ils détectent la racine du projet puis le dossier `data/`.
- **Dossier de données** : `data/` à la racine du projet (ex. `data/conduites/wMain.shp`, `data/pipe_ranking_v1_clear.csv`).
...
python script/database/row-data/read_schema.py
python script/database/row-data/describe_wmain_columns.py
python script/database/row-data/describe_wabandonedline_columns.py

# Depuis le dossier des scripts
cd script/database/row-data
...
Ces scripts sont **en lecture seule** et servent à la compréhension des données et à la préparation de la base SQL (voir `script/database/build-sqlite/`).
